main <- function() {
  ...
  if (exists("processing_in_progress", envir = .GlobalEnv) && get("processing_in_progress", envir = .GlobalEnv)) {
    message("⚠️ Скрипт уже запущен — прерываю, чтобы избежать рекурсии.")
  ...
  required_packages <- c(
    ...
    "ggrepel", "ggspatial", "ggmap", "maptiles", "terra"
  )
  ...
  # Загружаем скрипты по одному с выводом имени
  ...
  # Последовательное выполнение шагов с подробным логом
  ...
  message("🚀 Шаг 5: Загрузка VIIRS NOAA21")
