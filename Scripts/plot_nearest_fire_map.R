...
plot_nearest_fire_map <- function(fires_sf, places_sf, water_sf, output_path = "output/nearest_fire_map_ggplot.png") {
  ...
  required_packages <- c("ggplot2", "sf", "dplyr", "maptiles")
  ...
  # Находим ближайший пожар по расстоянию до населённого пункта
  ...
  # Расширяем bbox для визуализации
  ...
  # Получаем подложку тайлов с maptiles
  background <- maptiles::get_tiles(
    nearest_fire,
  ...
  # Построение карты
  p <- ggplot() +
    maptiles::layer_spatial(background) +  # Подложка карты
    ...
    labs(
      title = "🔥 Ближайший возгорание, населённый пункт и водоём",
      caption = paste0(
