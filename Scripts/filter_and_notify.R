# Функция-обёртка: фильтрует критические пожары, генерирует карту и отправляет уведомление в Telegram
filter_and_notify <- function(weather_day_df,
  ...
  # 2. Вычисляет минимальные расстояния до населённых пунктов и водоёмов
  ...
  # 3. Находит ближайшую точку с известным населённым пунктом
  ...
  # 4. Безопасно ищет имя региона среди нескольких вариантов названий колонок
  ...
  # 5. Повторно вычисляет уровень риска пожара
  factor_data <- calc_fire_risk_flag(weather_day_df)
  if (is.null(factor_data)) factor_data <- "Неизвестен"

  # 6. Генерирует карту ближайшего пожара, населённого пункта и водоёма
  plot_nearest_fire_map(fire_dist, get_all_places(), get_all_waterbodies())

  # 7. Формирует текст сообщения с данными о риске и ближайших объектах
  msg <- paste0(
  ...
  # 8. Отправляет текстовое сообщение в Telegram
  tryCatch({
  ...
  # 9. Отправляет карту изображением в Telegram, если файл существует
